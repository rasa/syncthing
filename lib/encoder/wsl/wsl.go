...
//go:generate go run maketables.go -o table.go

// Package wsl encodes FAT reserved characters using the Unicode characters
...
var wslDecodeTransformer = runes.Map(func(r rune) rune {
...
var wslEncodingTransformer = runes.Map(func(r rune) rune {
...
var wslPatternEncodingTransformer = runes.Map(func(r rune) rune {
	if r >= 0 && r < encoder.NumChars {
		return wslPatternEncodes[r]
	}

	return r
})

type wslEncoder struct{}

// NewDecoder returns a WSL decoder.
func (wslEncoder) NewDecoder() *encoding.Decoder {
	return &encoding.Decoder{
		Transformer: wslDecodeTransformer,
	}
}

// NewEncoder returns a WSL encoder.
func (wslEncoder) NewEncoder() *encoding.Encoder {
	return &encoding.Encoder{
		Transformer: wslEncodingTransformer,
	}
}

// WSL encodes FAT reserved characters using the Unicode characters
// \uf000-\uf0ff which falls in a Private Use Area (PUA) in the Basic
// Multilingual Plane.
var WSL encoding.Encoding = wslEncoder{}

type patternEncoder struct{}

// NewDecoder returns a WSLPattern decoder.
...
// NewEncoder returns a WSLPattern encoder.
func (patternEncoder) NewEncoder() *encoding.Encoder {
	return &encoding.Encoder{
		Transformer: wslPatternEncodingTransformer,
	}
}

// WSLPattern is an encoder that encodes reserved characters (other
// than '*' and '?') using the Unicode Private Use Area (PUA) plane.
var WSLPattern encoding.Encoding = patternEncoder{}
...
func IsDecoded(name string) bool {
	for _, r := range name {
		...
		if wslEncodes[r] >= consts.BaseRune {
			return true
		}
	}
...
func IsEncoded(name string) bool {
	for _, r := range name {
		...
		if wslEncodes[r&0xff] >= consts.BaseRune {
			return true
		}
	}
...
func MustEncode(name string) string {
	encoded, err := WSL.NewEncoder().String(name)
	if err != nil {
		panic("bug: wsl.encode: " + err.Error())
	}
	return encoded
}
...
func MustEncodePattern(pattern string) string {
	encodedPattern, err := WSLPattern.NewEncoder().String(pattern)
	if err != nil {
		panic("bug: wsl.encodePattern: " + err.Error())
	}
