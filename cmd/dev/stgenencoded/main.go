...
// Command stgenencoded generates files encoded by the WSL/Rclone encoders. The
...
const (
	...
	inWindowMsg = "You appear to be running in a Windows environment"
...
func main() {
	...
	flag.BoolVar(&opts.controls, "controls", false,
		`Generate files containing control characters (\x01-\x1f), too`)
	flag.BoolVar(&opts.backslash, "backslash", false,
	...
	switch encoderOpt {
	case "rclone":
		opts.encodes = rconsts.Encodes
		opts.encodeMap = rconsts.EncodeMap
	case "wsl":
		opts.encodes = wconsts.Encodes
		opts.encodeMap = wconsts.EncodeMap
	default:
		usage()
	}
	...
	switch mode {
	case "encoded":
		err = genEncodeds(opts, root)
	case "decoded":
		err = genDecodeds(opts, root)
	case "both":
		err = genEncodeds(opts, root)
		if err == nil {
			err = genDecodeds(opts, root)
...
func genEncodeds(opts opts, root string) error {
	for _, r := range opts.encodes + `\` {
		if !opts.controls && unicode.IsControl(r) {
			continue
		}
		if !opts.backslash && r == '\\' {
			continue
		}
		encoded := opts.encodeMap[r] // was: r | consts.BaseRune
...
func genDecodeds(opts opts, root string) error {
	if build.IsWindows {
