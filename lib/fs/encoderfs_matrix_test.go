...
var isFATMap = make(map[fsutil.VolumeType]bool, 0)
var isFAT bool
...
func testEncoderFileInfo(t *testing.T, fs Filesystem, want statResult, fi FileInfo) error {
	...
	if fs.Type() == FilesystemTypeFake || isFAT {
...
func testEncoderGetRegularRunes() []rune {
	...
	for r := rune(0); r <= maxRune; r++ {
		...
		if strings.ContainsRune(fat.Encodes, r) {
			continue
		}
		// Skip NUL (\x00), / (\x2f), \ (\x5c)
		if strings.ContainsRune(fat.Nevers, r) {
			continue
		}
