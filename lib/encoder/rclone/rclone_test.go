...
func testRcloneEncoder(t *testing.T, tests []encodeTest) {
	...
	for i, test := range tests {
		...
		for _, length := range getLengths() {
			...
			if got != want {
				t.Errorf("Test %d: Rclone.Encode(%q) got %+q; want %+q (%d vs %d bytes)", j, ins[:cut], got[:cut], want[:cut], len(got), len(want))
			}
			...
			if got != want {
				t.Errorf("Test %d: Encode(%+q) got %+q; want %+q (%d vs %d bytes)", j, ins[:cut], got[:cut], want[:cut], len(got), len(want))
			}

			got = rclone.MustEncode(ins)
			if got != want {
				t.Errorf("Test %d: MustEncode(%+q) got %+q; want %+q (%d vs %d bytes)", j, ins[:cut], got[:cut], want[:cut], len(got), len(want))
			}
