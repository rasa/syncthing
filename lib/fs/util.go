...
func WindowsInvalidPath(name string, encoderType EncoderType, allowReservedFilenames bool) error {
	switch encoderType {
	case EncoderTypeUnset, EncoderTypeNone:
		return WindowsInvalidFilename(name, allowReservedFilenames)
	case EncoderTypeFat:
