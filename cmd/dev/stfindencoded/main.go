...
func find(mode Mode, root string, long bool) {
	...
	_ = vfs.Walk(".", func(name string, _ fs.FileInfo, err error) error {
		...
		case modeDecoded:
			if !wsl.IsDecoded(path) {
				return nil
			}
		case modeBoth:
			if !wsl.IsEncoded(name) && !wsl.IsDecoded(path) {
				return nil
			}
		}
		found++
...
func findDuplicates(root string, fixMode FixMode) {
	...
	err := vfs.Walk(".", func(name string, info fs.FileInfo, err error) error {
		...
		for _, file := range files {
			if !wsl.IsDecoded(file) {
				continue
			}
			key := wsl.MustDecode(file)
			decodes[key] = true
		}
		...
		for _, eFile := range files {
			dFile := wsl.MustDecode(eFile)
			if !wsl.IsEncoded(eFile) {
				if wsl.IsDecoded(eFile) {
					if !decodes[dFile] {
						stats.decodes++
					}
				} else {
