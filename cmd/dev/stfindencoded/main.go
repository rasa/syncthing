...
// Command stfindencoded finds files encoded by the Rclone/WSL encoders. The
// tool has reduced functionality when run in Windows, or WSL, as you cannot
// create pre-encoded (decoded) filenames in these environments.
package main
...
type opts struct {
	...
	fixMode    FixMode
	long       bool
	IsDecoded  func(name string) bool
	IsEncoded  func(name string) bool
	MustDecode func(name string) string
}
...
func main() {
	...
	flag.BoolVar(&opts.long, "long", false,
		"Use a long listing format")
	flag.StringVar(&encoderOpt, "encoder", encoder.DefaultEncoder,
		"Set encoder: rclone, wsl")
	...
	switch defFixMode {
	case "manual":
		opts.fixMode = fixModeManual
	case "decoded":
		opts.fixMode = fixModeDecoded
	case "encoded":
		opts.fixMode = fixModeEncoded
	case "newer":
		opts.fixMode = fixModeNewer
	case "older":
		opts.fixMode = fixModeOlder
	default:
		usage()
	}
	...
	switch mode {
	case "encoded":
		opts.mode = modeEncoded
	case "decoded":
		opts.mode = modeDecoded
	case "both":
		opts.mode = modeBoth
	case "duplicates":
		opts.mode = modeDuplicates
	case "fix":
		opts.mode = modeFix
	default:
		usage()
	...
	switch encoderOpt {
	case "rclone":
		opts.IsDecoded = rclone.IsDecoded
		opts.IsEncoded = rclone.IsEncoded
		opts.MustDecode = rclone.MustDecode
	case "wsl":
		opts.IsDecoded = wsl.IsDecoded
		opts.IsEncoded = wsl.IsEncoded
		opts.MustDecode = wsl.MustDecode
	default:
		usage()
	}

	switch opts.mode {
	case modeEncoded, modeDecoded, modeBoth:
		for _, arg := range args {
			find(opts, arg)
		}
	case modeDuplicates:
		opts.fixMode = fixModeListOnly
		for _, arg := range args {
			findDuplicates(opts, arg)
		}
	case modeFix:
...
func find(opts opts, root string) {
	msg := ""
	switch opts.mode {
	case modeEncoded:
	...
	regex := "[" + regexp.QuoteMeta(encoder.Nevers) + "]"
	...
	_ = vfs.Walk(".", func(name string, _ fs.FileInfo, err error) error {
		...
		switch opts.mode {
		case modeEncoded:
			if !opts.IsEncoded(name) {
				return nil
			}
		case modeDecoded:
			if !opts.IsDecoded(path) {
				return nil
			}
		case modeBoth:
			if !opts.IsEncoded(name) && !opts.IsDecoded(path) {
				return nil
			}
		}
		found++
...
func findDuplicates(opts opts, root string) {
	if oswsl.IsWSL() {
	...
	err := vfs.Walk(".", func(name string, info fs.FileInfo, err error) error {
		...
		for _, file := range files {
			if !opts.IsDecoded(file) {
				continue
			}
			key := opts.MustDecode(file)
			decodes[key] = true
		}
		...
		for _, eFile := range files {
			dFile := opts.MustDecode(eFile)
			if !opts.IsEncoded(eFile) {
				if opts.IsDecoded(eFile) {
					if !decodes[dFile] {
						stats.decodes++
					}
				} else {
