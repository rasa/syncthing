...
func (fs *fakeFS) Lstat(name string) (FileInfo, error) {
	...
	presentedName := filepath.Base(name)
	info := &fakeFileInfo{*entry, presentedName}
	return info, nil
}
...
func (fs *fakeFS) rooted(rel string) (string, error) {
	...
	if fs.encoderType == EncoderTypeFat || fs.encoderType == EncoderTypeWindows {
		if fs.pattern {
...
func (fs *fakeFS) isValid(name string) bool {
	...
	default:
		l.Warnf("Unexpected volume type %v, please update fakefs.go's isValid()", fs.volumeType)
	}

	switch fs.encoderType {
	case EncoderTypeUnset, EncoderTypeNone:
	case EncoderTypeFat, EncoderTypeWindows:
	default:
	...
	if (fs.encoderType == EncoderTypeFat || fs.encoderType == EncoderTypeWindows) && fat.IsEncoded(name) {
		return false
	}
