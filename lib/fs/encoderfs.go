...
func EncoderTypeOption(encoderType encoder.EncoderType) Option {
	switch encoderType {
	...
	case encoder.EncoderTypeRclone:
		return new(OptionRcloneEncoder)
	case encoder.EncoderTypeWSL:
		return new(OptionWSLEncoder)
	default:
...
func (f *encoderFS) EncoderType() encoder.EncoderType {
...
// DefaultEncoderType returns the default encoder type for the OS. On Windows
// and Android we default to the Rclone encoder per @calmh's comment at
// https://github.com/syncthing/syncthing/issues/9539#issuecomment-2141394377
// On all other systems, including under WSL on Windows, we default to the None
// encoder.
func DefaultEncoderType() encoder.EncoderType {
	if !build.IsWindows && !build.IsAndroid {
		return encoder.EncoderTypeNone
	}
	switch encoder.DefaultEncoder {
	case "rclone":
		return encoder.EncoderTypeRclone
	case "wsl":
		return encoder.EncoderTypeWSL
	}
	panic("bug: unknown encoder: " + encoder.DefaultEncoder)
}
