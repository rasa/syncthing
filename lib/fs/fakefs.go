...
//     encoder=s  "wsl" (or "rclone") makes filesystem fail on encoded filenames
//
...
func (fs *fakeFS) rooted(rel string) (string, error) {
	...
	case encoder.EncoderTypeRclone:
		if fs.pattern {
			return rclone.EncodePattern(rel)
		}

		rv, err := rclone.Encode(rel)
		return rv, err
	case encoder.EncoderTypeWSL:
		if fs.pattern {
			return wsl.EncodePattern(rel)
		}

		rv, err := wsl.Encode(rel)
		return rv, err
	default:
		panic("bug: Unknown encoderType: " + fs.encoderType.String())
	}
...
func (fs *fakeFS) unrooted(path string) string {
	...
	case encoder.EncoderTypeNone:
		// noop
	case encoder.EncoderTypeRclone:
		return rclone.MustDecode(path)
	case encoder.EncoderTypeWSL:
		return wsl.MustDecode(path)
	default:
		panic(fmt.Sprintf("Unexpected encoderType %v", fs.encoderType))
	}
...
// false. If the WSL encoder is enabled (`encoder=wsl`), and the filename
// contains encoded characters, isValid returns false, as the real WSL encoder
...
func (fs *fakeFS) isValid(name string) bool {
	...
	// The Rclone encoder rejects filenames containing encoded characters, even on
	// Ext/ext-like volumes.
	if fs.encoderType == encoder.EncoderTypeRclone && rclone.IsEncoded(name) {
		return false
	}

	// The WSL encoder rejects filenames containing encoded characters, even on
	// Ext/ext-like volumes.
	if fs.encoderType == encoder.EncoderTypeWSL && wsl.IsEncoded(name) {
		return false
	}
	...
	if !wsl.IsDecoded(name) && !rclone.IsDecoded(name) {
		return true
	}

	// The Rclone encoder accepts filenames with reserved characters.
	if fs.encoderType == encoder.EncoderTypeRclone && rclone.IsDecoded(name) {
		return true
	}

	// The WSL encoder accepts filenames with reserved characters.
	if fs.encoderType == encoder.EncoderTypeWSL && wsl.IsDecoded(name) {
		return true
	}

	// Without an encoder, FAT filesystems reject filenames with reserved
	// characters.
	return false
}
