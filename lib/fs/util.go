...
// When reservedFilenames is true, filenames that end in a period or space, and
// reserved Windows filenames such as CON or NUL.txt will be saved on disk.
// These files can be difficult for non-technical users to work with. We can
// address this by encoding these filenames instead, if desired.
func WindowsInvalidPath(name string, encoderType encoder.EncoderType, reservedFilenames bool) error {
	switch encoderType {
	case encoder.EncoderTypeRclone, encoder.EncoderTypeWSL:
