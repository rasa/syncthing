...
var testResultMatrix = map[encoder.EncoderType]map[encoder.EncoderType]map[srcType]dstType{
	encoder.EncoderTypeNone: { // src
	...
	encoder.EncoderTypeWSL: { // src
		encoder.EncoderTypeNone: { // dst
...
func TestEncoderSync(tttt *testing.T) {
	...
	for _, srcEncoderTypeID := range srcEncoderTypeIDs {
		srcEncoderType := encoder.EncoderType(srcEncoderTypeID)
		...
		tttt.Run(tttName, func(ttt *testing.T) {
			...
			for _, dstEncoderTypeID := range dstEncoderTypeIDs {
				dstEncoderType := encoder.EncoderType(dstEncoderTypeID)
...
func testEncoderSyncAll(t *testing.T, srcEncoder, dstEncoder encoder.EncoderType, srcType srcType) {
	t.Helper()
	...
	if build.IsWindows {
		key := ""
		if srcEncoder == encoder.EncoderTypeNone && srcType == srcTypeDecoded {
			key = "src"
		}
		if dstEncoder == encoder.EncoderTypeNone && dstType == dstTypeDecoded {
			key = "dst"
		}
		if key != "" {
...
func testEncoderSyncOneSideToOther(t *testing.T, srcEncoder, dstEncoder encoder.EncoderType, srcType srcType, dstType dstType) {
	t.Helper()
...
// sync_2dev_test.go.
func testEncoderSyncMergeTwoDevices(t *testing.T, srcEncoder, dstEncoder encoder.EncoderType, srcType srcType, dstType dstType) {
	t.Helper()
...
func testEncoderSyncTwoDevicesFolders(ctx context.Context, t *testing.T, srcDir, dstDir string, srcEncoderType, dstEncoderType encoder.EncoderType) error {
	t.Helper()
...
// srcType.
func srcPrefixes(srcType srcType, srcEncoder encoder.EncoderType) string {
	switch srcType {
	case srcTypeDecoded:
...
// dstType.
func dstPrefixes(dstType dstType, dstEncoder encoder.EncoderType) string {
	switch dstType {
	case dstTypeDecoded:
...
func fatDecodes(enc encoder.EncoderType) string {
	var encodes string
	...
	default:
		panic("bug: unexpected encoderType " + enc.String())
	}
...
func encodedChars(enc encoder.EncoderType) string {
	var encodes string
...
func wanted(srcEncoder encoder.EncoderType, dstType dstType, srcCreated, dstCreated int) int {
	var want int
	switch dstType {
	...
	case dstTypeRejectEncoded:
		...
		if build.IsWindows { // && srcEncoder == encoder.EncoderTypeWSL
			if verbose {
