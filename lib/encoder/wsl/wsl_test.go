...
func TestWSLDecoderDecodes(t *testing.T) {
	t.Parallel()
	testWSLDecoder(t, decodeTests)
}

func TestWSLDecoderLatin1Chars(t *testing.T) {
	t.Parallel()
	tests := make([]decodeTest, 0, encoder.NumChars)
	for r := rune(0); r < rune(encoder.NumChars); r++ {
		ins := string(r)
		out := ins
	...
	testWSLDecoder(t, tests)
}

func TestWSLDecoderWSLChars(t *testing.T) {
	t.Parallel()
	tests := make([]decodeTest, 0, encoder.NumChars)
	for r := rune(0); r < rune(encoder.NumChars); r++ {
...
func TestWSLDecoderEncodes(t *testing.T) {
	t.Parallel()
...
func TestWSLEncoderEncodes(t *testing.T) {
	t.Parallel()
	testWSLEncoder(t, encodeTests)
}

func TestWSLEncoderLatin1Chars(t *testing.T) {
	t.Parallel()
	tests := make([]encodeTest, 0, encoder.NumChars)
	...
	testWSLEncoder(t, tests)
}

func TestWSLEncoderWSLChars(t *testing.T) {
	t.Parallel()
	tests := make([]encodeTest, 0, encoder.NumChars)
...
func TestWSLEncoderDecodes(t *testing.T) {
	t.Parallel()
...
func TestWSLPatternEncoder(t *testing.T) {
	t.Parallel()
	enc := wsl.WSLPattern.NewEncoder()
	...
	for i, test := range encodeTests {
		...
		for _, length := range getLengths() {
			...
			if err != nil {
				t.Errorf("Test %d: WSLPattern.Encode(%+q) unexpected error; %v", j, ins, err)
			}
			if got != want {
				t.Errorf("Test %d: WSLPattern.Encode(%+q) got %+q; want %+q (%d vs %d bytes)", j, ins, got, want, len(want), len(want))
			}

			got, err = wsl.EncodePattern(ins)
			if err != nil {
...
func TestWSLPatternEncoderSpecific(t *testing.T) {
	t.Parallel()
	enc := wsl.WSLPattern.NewEncoder()
	...
	for i, test := range patternEncodeTests {
		...
		for _, length := range getLengths() {
			...
			if got != out {
				t.Errorf("Test %d: WSLPattern.Encode(%+q) got %+q; want %+q (%d vs %d bytes)", j, ins, got, out, len(got), len(out))
			}
		}
	}
}

func TestWSLIsDecoded(t *testing.T) {
	t.Parallel()
	testWSLDecoder(t, decodeTests)
}

func testWSLDecoder(t *testing.T, tests []decodeTest) {
	t.Helper()
	...
	for i, test := range tests {
		...
		for _, length := range getLengths() {
			...
			if err != nil {
				t.Errorf("Test %d: WSL.Decode(%q) got %v, want %v", j, ins, err, nil)
			}
			if got != want {
				t.Errorf("Test %d: WSL.Decode(%q) got %+q; want %+q (%d vs %d bytes)", j, ins, got, want, len(got), len(want))
			}

			got, err = wsl.Decode(ins)
			if err != nil {
...
func testWSLEncoder(t *testing.T, tests []encodeTest) {
	t.Helper()

	enc := wsl.WSL.NewEncoder()
	...
	for i, test := range tests {
		...
		for _, length := range getLengths() {
			...
			got, err := enc.String(ins)
			if err != nil {
				t.Errorf("Test %d: WSL.Encode(%+q) got %v, want %v", j, ins, err, nil)
			}
			if got != want {
				t.Errorf("Test %d: WSL.Encode(%q) got %+q; want %+q (%d vs %d bytes)", j, ins, got, want, len(got), len(want))
			}

			got, err = wsl.Encode(ins)
			if err != nil {
...
func getLengths() []int {
	lengths := make([]int, 0)
	maxLenRaw := os.Getenv("STTESTWSLMAXLEN")
	if maxLenRaw != "" {
...
func BenchmarkWslDecoder(b *testing.B) {
	b.ReportAllocs()
...
func BenchmarkWslEncoder(b *testing.B) {
	b.ReportAllocs()
...
func BenchmarkWslPatternEncoder(b *testing.B) {
	b.ReportAllocs()
...
/*
Benchmark results for various defaultDecodeTransformer functions in wsl.go:

// BenchmarkWslDecoder-4            1973989              6487 ns/op
//
var defaultDecodeTransformer = runes.Map(func(r rune) rune {
	if r < consts.BaseRune || r >= (consts.BaseRune+encoder.NumChars) {
		return r
	}
	if wslEncodes[r&^consts.BaseRune] >= consts.BaseRune {
...
// BenchmarkWslDecoder-4            1861136              6589 ns/op
//
var defaultDecodeTransformer = runes.Map(func(r rune) rune {
	if r < consts.BaseRune || r >= (consts.BaseRune+encoder.NumChars) {
		return r
...
// BenchmarkWslDecoder-4            1648190              6779 ns/op
//
var defaultDecodeTransformer = runes.Map(func(r rune) rune {
	if r < consts.BaseRune || r >= (consts.BaseRune+encoder.NumChars) {
		return r
...
// BenchmarkWslDecoder-4            1665566              7235 ns/op
//
var decodeMap map[rune]rune
func init() {
	decodeMap = make(map[rune]rune, len(wslEncodes))
...
// BenchmarkWslDecoder-4            1371726              8911 ns/op
//
var defaultDecodeTransformer = runes.Map(func(r rune) rune {
	if strings.ContainsRune(wslEncodes, r) {
		return r &^ consts.BaseRune
	}
})
