...
func (f *encoderFS) CreateSymlink(target, name string) error {
	// BasicFilesystem's CreateSymlink encodes name, but not the target
	// (as it's already relative), so we need to encode it.
	// works: encode, fail: regular and decode
...
func (f *encoderFS) SameFile(fi1, fi2 FileInfo) bool {
	...
	if ok1 && ok2 {
		// It would preferable to use
		//  f.Filesystem.SameFile(ef1.osFileInfo(), ef2.osFileInfo())
		// here, but doing so gives this error:
		//  cannot use ef1.osFileInfo() (value of type "io/fs".FileInfo) as
		//  FileInfo value in argument to f.Filesystem.SameFile: "io/fs".FileInfo does
		//  not implement FileInfo (missing method Group)
		// so we are forced to use os.SameFile(), but this shouldn't be a concern
		// as this exactly what basicfs's SameFile() does.
		return os.SameFile(ef1.osFileInfo(), ef2.osFileInfo())
	}

	// This should never occur, but it can't hurt to check.
	bf1, ok1 := fi1.(basicFileInfo)
	bf2, ok2 := fi2.(basicFileInfo)
	if ok1 && ok2 {
		// Can't use f.Filesystem.SameFile() here either.
		return os.SameFile(bf1.osFileInfo(), bf2.osFileInfo())
	}

	ff1, ok1 := fi1.(*fakeFileInfo)
	ff2, ok2 := fi2.(*fakeFileInfo)
	if ok1 && ok2 {
		// Here we can...
		return f.Filesystem.SameFile(ff1, ff2)
	}
