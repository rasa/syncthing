...
func TestEncoderMatrix(tttt *testing.T) {
	...
	for filesystemTypeID := range filesystemTypes {
		...
		tttt.Run(tttName, func(ttt *testing.T) {
			// NoneEncoder / WSLEncoder / RcloneEncoder
...
func runEncoderTests(t *testing.T, rootURI string, filesystemType FilesystemType, volumeType fsutil.VolumeType, encoderType EncoderType) {
	t.Helper()
...
func runEncoderTest(t *testing.T, fs Filesystem, encTest encoderTest, testFunc testFunc, encoderType EncoderType) {
	t.Helper()
	testName := path.Base(t.Name())

	regularFiles, decodedFiles, encodedFiles := testEncoderGetTestFilenames(encoderType)

	regularFiles = []string{} // @TODO REMOVE ME
	encodedFiles = []string{} // @TODO REMOVE ME
...
func testEncoderGetTestFilenames(encoderType EncoderType) ([]string, []string, []string) {
	...
	for _, r = range testEncoderGetWSLRunes(encoderType) {
...
func testEncoderGetRegularRunes(encoderType EncoderType) []rune {
	var encodes string
	var nevers string
	switch encoderType {
	case EncoderTypeWSL:
		encodes = wconsts.Encodes
		nevers = wconsts.Nevers
	case EncoderTypeRclone:
		encodes = rconsts.Encodes
		nevers = rconsts.Nevers
	case EncoderTypeUnset, EncoderTypeNone:
		return []rune{}
	default:
		panic("bug: unknown encoder: " + encoderType.String())
...
func testEncoderGetWSLRunes(encoderType EncoderType) []rune {
	var input string
	switch encoderType {
	case EncoderTypeWSL:
...
func testEncoderGetGlobFiles(fs Filesystem, name string, maxNames int, recursive bool, encoderType EncoderType) ([]string, error) {
	regularFiles, decodedFiles, encodedFiles := testEncoderGetTestFilenames(encoderType)
