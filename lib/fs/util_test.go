...
func TestWindowsInvalidFilename(t *testing.T) {
	...
	for _, tc := range cases {
		err := WindowsInvalidFilename(tc.name, tc.reservedFilenames)
		if !errors.Is(err, tc.err) {
			t.Errorf("For %q/%v, got %v, expected %v", tc.name, tc.reservedFilenames, err, tc.err)
		}
