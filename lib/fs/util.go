...
func WindowsInvalidPath(name string, encoderType encoder.EncoderType) error {
	switch encoderType {
	case encoder.EncoderTypeRclone, encoder.EncoderTypeWSL:
		// The Rclone/WSL encoders "saves" files containing invalid characters.
		// It still allows filenames that end in a period or space, and reserved
		// Windows filenames such as CON or NUL.txt.
