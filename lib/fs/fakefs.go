...
func (fs *fakeFS) isValid(name string) bool {
	...
	if fs.encoderType == EncoderTypeWSL && wsl.IsEncoded(name) {
		return false
	}

	// The Rclone encoder rejects filenames containing encoded characters, even on
	// Ext/ext-like volumes.
	if fs.encoderType == EncoderTypeRclone && rclone.IsEncoded(name) {
		return false
	}
	...
	if fs.encoderType == EncoderTypeWSL && wsl.IsDecoded(name) {
		return true
	}

	// The Rclone encoder accepts filenames with reserved characters.
	if fs.encoderType == EncoderTypeRclone && rclone.IsDecoded(name) {
		return true
	}
