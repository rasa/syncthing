...
import (
	...
	oswsl "github.com/syncthing/syncthing/lib/osutil/wsl"
...
var encoderTestMatrix = map[fsutil.VolumeType]map[encoder.EncoderType]encoderTest{
	fsutil.VolumeTypeExt: {
		encoder.EncoderTypeNone: {
			decodeOK: true,
			encodeOK: true,
		},
		encoder.EncoderTypeRclone: {
			decodeOK: true,
			encodeOK: false,
		},
		encoder.EncoderTypeWSL: {
			decodeOK: true,
			encodeOK: false,
		},
	},
...
var (
	msgNoSymlinksFS      sync.Once
	msgNoSymlinksWindows sync.Once
...
func TestEncoderMatrix(tttt *testing.T) {
	_, ok := unwrapFilesystem[*noneEncoderFS](testFs)
	if !ok {
	...
	if ok {
		...
		// and again using an wslEncoderFS. So we can skip this test in
	...
	for filesystemTypeID := range filesystemTypes {
		...
		tttName := title(fmt.Sprintf("%v", filesystemType)) + "FS"
		tttt.Run(tttName, func(ttt *testing.T) {
			// NoneEncoder / RcloneEncoder / WSLEncoder
			for encoderTypeID := range encoderTypes {
				encoderType := encoder.EncoderType(encoderTypeID)
				if encoderType == encoder.EncoderTypeUnset {
					continue
				}
				...
				ttt.Run(ttName, func(tt *testing.T) {
					...
					for _, volumeType := range fsutil.VolumeTypes {
						...
						tt.Run(tName, func(t *testing.T) {
							...
							runEncoderTests(t, rootURI, filesystemType, volumeType, encoderType)
						})
					}
				})
			}
...
func runEncoderTests(t *testing.T, rootURI string, filesystemType FilesystemType, volumeType fsutil.VolumeType, encoderType encoder.EncoderType) {
	t.Helper()

	encoderTests := encoderTestMatrix[volumeType]
	encTest := encoderTests[encoderType]
	...
	t.Run("DirNames0", func(t *testing.T) {
		...
		runEncoderTest(t, fs, encoderTest{true, true}, testFunc, encoderType)
	})
	...
	t.Run("Create", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	maxNames := -1
	testFunc = func(name string) error {
		names, err := testEncoderGetGlobFiles(fs, name, maxNames, false, encoderType)
		if err != nil {
			return err
		}
	...
	t.Run("DirNames", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("Chtimes", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })

	// Mkdir
	testFunc = func(name string) error {
		return fs.Mkdir(name, 0o775)
	}
	t.Run("Mkdir", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("MkdirAll", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("Open", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("OpenFile", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("Remove", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("RemoveAll", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("Rename", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("Stat", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("Stat-Dir", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	testFunc = func(name string) error {
		...
		for _, test := range tests {
			names, err := testEncoderGetGlobFiles(fs, name, maxNames, test.recursive, encoderType)
			if err != nil {
				return err
			}
	...
	t.Run("Glob", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("File.Stat", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("SameFileDiff", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("SameFileSame", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("Chmod", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("Lchown", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })

	if fs.Type() == "fake" {
		msgNoSymlinksFS.Do(func() {
	...
	if build.IsWindows {
		msgNoSymlinksWindows.Do(func() {
			t.Skipf("Skipping symlink tests as Syncthing doesn't support symlinks on %v", runtime.GOOS)
		})
	...
	t.Run("CreateSymlink", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("ReadSymlink", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("Lstat", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("Lchown-Symlink", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
	...
	t.Run("Stat-Symlink", func(t *testing.T) { runEncoderTest(t, fs, encTest, testFunc, encoderType) })
}

func runEncoderTest(t *testing.T, fs Filesystem, encTest encoderTest, testFunc testFunc, encoderType encoder.EncoderType) {
	t.Helper()
	testName := path.Base(t.Name())

	regularFiles, decodedFiles, encodedFiles := testEncoderGetTestFilenames(encoderType)
	...
	for _, name := range regularFiles {
		...
		if err != nil {
			...
			t.Fatalf("Regular:  %q: %q: got %q (%T), want %v",
	...
	for _, name := range encodedFiles {
		...
		if encTest.encodeOK != ok {
			t.Fatalf("Encoded:  %q: %q: got %q (%T), want %q",
	...
	if build.IsWindows || oswsl.IsWSL() {
	...
	for _, name := range decodedFiles {
		...
		if encTest.decodeOK != ok {
			t.Fatalf("Decoded: %q: %q: got %q (%T), want %q",
...
func testEncoderGetTestFilenames(encoderType encoder.EncoderType) ([]string, []string, []string) {
	var regularFiles []string
	var decodedFiles []string
	var encodedFiles []string

	var r rune
	for _, r = range testEncoderGetRegularRunes(encoderType) {
	...
	for _, r = range testEncoderGetEncodedRunes(encoderType) {
	...
	for _, r = range testEncoderGetEncodedRunes(encoderType) {
		...
		case encoder.EncoderTypeRclone:
			encoded = rconsts.EncodeMap[r]
		case encoder.EncoderTypeWSL:
			encoded = wconsts.EncodeMap[r]
...
func testEncoderGetRegularRunes(encoderType encoder.EncoderType) []rune {
	var encodes string
	switch encoderType {
	case encoder.EncoderTypeRclone:
	...
	for r := rune(0); r <= maxRune; r++ {
		...
		if strings.ContainsRune(encodes, r) {
			continue
		}
...
func testEncoderGetEncodedRunes(encoderType encoder.EncoderType) []rune {
	var input string
	switch encoderType {
	case encoder.EncoderTypeRclone:
		input = rconsts.Encodes
	case encoder.EncoderTypeWSL:
		input = wconsts.Encodes
	case encoder.EncoderTypeUnset, encoder.EncoderTypeNone:
		// noop
	default:
		panic("bug: unknown encoder: " + encoderType.String())
...
func testEncoderGetGlobFiles(fs Filesystem, name string, maxNames int, recursive bool, encoderType encoder.EncoderType) ([]string, error) {
	regularFiles, decodedFiles, encodedFiles := testEncoderGetTestFilenames(encoderType)
...
func tempDirEnv(t *testing.T, envvar string) (string, error) {
	...
	if envvar != "" {
		...
		if dir != "" {
			...
			if err != nil {
				t.Fatalf("Cannot use directory %q (%v): %v", dir, envvar, err)
...
func fsutilTempDir(dir string) (string, error) {
	...
	dir, err = os.MkdirTemp(dir, "syncthing*.tmp")
	if err != nil {
