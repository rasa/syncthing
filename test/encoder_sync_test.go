...
var (
	...
	testNumber   = 0
	skippedTests = 0
	totalTests   = len(testResultMatrix) * 2 * len(srcTypes) *
		numberOfSubTests
...
// tests, but tests 1, 2, 5, and 6 are skipped, as they can't be performed, as
...
func TestEncoderSync(tttt *testing.T) {
	...
	// runs last.
	srcEncoderTypeIDs := make([]fs.EncoderType, 0, len(testResultMatrix))
	for id := range testResultMatrix {
	...
	sort.Slice(srcEncoderTypeIDs, func(i, j int) bool {
		return int(srcEncoderTypeIDs[i]) > int(srcEncoderTypeIDs[j])
	})
	...
	for _, srcEncoderTypeID := range srcEncoderTypeIDs {
		...
		tttt.Run(tttName, func(ttt *testing.T) {
			for dstEncoderTypeID := range testResultMatrix {
